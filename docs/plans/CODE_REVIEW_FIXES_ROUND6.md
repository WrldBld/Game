...
### CR6-1.2 - Action-Based Time Suggestions Must Be DM-Approved

**Severity**: CRITICAL (UPDATED)

**Issue**: "Auto" advancement conflicts with the intended game model: time should advance only as a result of a meaningful in-game event **and** a DM decision (approve/modify), or via an explicit DM time operation (advance/set/skip).

**Resolution**:

- Engine no longer auto-advances time from player actions.
- Action-based time always produces a time suggestion for DM approval (when time costs apply).
- DM responses to time suggestions advance time and broadcast `GameTimeAdvanced`.
