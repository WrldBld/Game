...
- **Inbound ports (driving ports)**
  ...
  - Define what the application _offers_ to the outside world.
...
- **Outbound ports (driven ports)**
  ...
  - Implemented by: adapters (outer layer). If a trait is implemented inside the application layer, it is usually **not a port** in the hexagonal sense.
...
### How to classify a trait (mechanical rubric)

When a trait’s name is confusing (especially anything ending in `*ServicePort`), classify it by _who calls it_ and _who implements it_:

1. If adapters/handlers/UI **call** the trait and application code **implements** it → **Inbound port**.
2. If application code **depends on** the trait and adapters **implement** it → **Outbound port**.
3. If application code both **calls and implements** it (purely internal collaboration) → **Not a port**.

This rubric avoids semantic drift where “service-ish” traits end up in the wrong folder.
...
### `*ServicePort` naming guidance (source of common confusion)

The suffix `*ServicePort` does **not** determine direction.

- If the trait is part of the app’s public API (adapters call it), it is **inbound** even if it “feels like a service” (e.g., an `Asset...` API called by HTTP routes).
- If the trait represents an external dependency (app calls it), it is **outbound** even if it “feels like a service” (e.g., prompt resolution, config, LLM, filesystem, DB).
- If it’s an app-internal helper abstraction, keep it in `engine-app`/`player-app` as an internal trait (do not model it as a port).
...
If it is used _inside the engine_ across app↔adapter boundaries, it belongs with the port that owns it:
...
- Inbound ports define what the application _offers_ - use cases _implement_ them
- Outbound ports define what the application _needs_ - use cases _depend on_ them
