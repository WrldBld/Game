...
**Status**: ✅ Complete (Basic Structure - Refinement Pending)
...
│  │ ▼ Actantial Roles / ▼ Secret Behavior                              │   │
│  └─────────────────────────────────────────────────────────────────────┘   │
│                                                                             │
...
| Step | Status | Notes |
|------|--------|-------|
| 1. Protocol DTOs | ✅ Complete | DTOs + messages + placeholder handlers |
| 2. Service Layer | ✅ Complete | Mutation methods + request types + goal methods |
| 3. HTTP Routes | ✅ Complete | want_routes.rs with full CRUD + actantial views |
| 4. WebSocket Handlers | ✅ Complete | 16 handlers + suggestion methods + queue routing |
| 5. Player-App Services | ✅ Complete | ActantialService with HTTP-based CRUD |
| 6. UI Components | ✅ Complete | Basic MotivationsTab with all sub-components |
| 7. Character Form Integration | ✅ Complete | MotivationsTab section added |
| 8. Session Message Handler | ✅ Complete | All handlers with logging (TODO for state updates) |
| 9. Common Goals Settings | ✅ Complete | 12 default goals in domain layer |
| 10. Documentation | ✅ Complete | character-system.md + plan updated |
| Final Check | ✅ Complete | cargo check + arch-check pass |
...
- [x] **Sprint 5**: CharacterPicker component for actantial view targets (replaces manual ID entry)

### Session Message Handler ✅
- [x] All actantial message types have handlers with logging
- [x] **Sprint 5**: Added `actantial_refresh_counter` signal to GameState
- [x] **Sprint 5**: All 16 message handlers trigger `trigger_actantial_refresh()`
- [x] **Sprint 5**: MotivationsTab subscribes to global refresh signal
...
### Sprint 5 Additions
- `player-ui/.../state/game_state.rs` - Added `actantial_refresh_counter` signal
- `player-ui/.../handlers/session_message_handler.rs` - 16 handlers trigger refresh
- `player-ui/.../components/common/character_picker.rs` - New component (~290 lines)
- `player-ui/.../components/common/mod.rs` - Export CharacterPicker
...
- **Sprint 5**: CharacterPicker dropdown for NPC/PC selection (searchable, type badges)
- Reason input (optional)
- Removable badges for existing views with click-to-remove
- Service calls via ActantialService HTTP endpoints

### Pending (Future Work)
- Target selection UI (picker for items, goals) - characters done via CharacterPicker
- Multi-DM real-time state sync via WebSocket - currently uses refresh pattern
