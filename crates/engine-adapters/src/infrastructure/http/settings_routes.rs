...
pub fn settings_routes() -> Router<Arc<AdapterState>> {
    Router::new()
        ...
        .route(
            "/api/worlds/{world_id}/settings",
            put(update_world_settings),
        )
        .route(
            "/api/worlds/{world_id}/settings/reset",
            post(reset_world_settings),
        )
        .route(
            "/api/worlds/{world_id}/settings",
            delete(delete_world_settings),
        )
}
...
async fn get_world_settings(
    ...
    Ok(Json(state.app.settings_service.get_for_world(world_id).await))
}
