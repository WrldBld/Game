...
pub fn app_event_to_domain_event(event: AppEvent) -> Result<DomainEvent, DomainEventConversionError> {
    match event {
            AppEvent::StoryEventCreated {
                story_event_id,
                world_id,
            ...
            AppEvent::ChallengeResolved {
                ...
                challenge_id: challenge_id.map(|id| parse_id::<ChallengeId>(&id)).transpose()?,
                challenge_name,
                world_id: parse_id::<WorldId>(&world_id)?,
