...
pub async fn extract_context_opt(state: &AppState, client_id: Uuid) -> Option<UseCaseContext> {
    let conn = state
        .world_connection_manager
...
pub async fn extract_dm_context_opt(state: &AppState, client_id: Uuid) -> Option<UseCaseContext> {
