...
use wrldbldr_engine_app::application::use_cases::{
    NpcMotivation, RequestSceneChangeInput, SceneApprovalDecision, SceneApprovalDecisionInput,
    SceneChangeResult, UpdateDirectorialInput,
};
use wrldbldr_protocol::{CharacterPosition, DirectorialContext, ServerMessage};

use super::common::{error_msg, extract_context_opt, extract_dm_context_opt};
...
pub async fn handle_directorial_update(
    ...
    let ctx = match extract_dm_context_opt(state, client_id).await {
        Some(c) => c,
        None => {
...
pub async fn handle_approval_decision(
    ...
    let ctx = match extract_dm_context_opt(state, client_id).await {
        ...
        None => {
            return Some(error_msg(
                "NOT_AUTHORIZED",
                "Only the DM can approve responses",
            ))
        }
    };
    let input = SceneApprovalDecisionInput {
        request_id,
...
fn convert_scene_result(result: SceneChangeResult) -> Option<ServerMessage> {
    ...
    Some(ServerMessage::SceneUpdate {
        ...
        characters: result
            .characters
            .into_iter()
            .map(|c| wrldbldr_protocol::CharacterData {
                id: c.id,
                name: c.name,
        ...
        interactions: result
            .interactions
            .into_iter()
            .map(|i| wrldbldr_protocol::InteractionData {
                id: i.id,
                name: i.name,
...
fn convert_decision(d: wrldbldr_protocol::ApprovalDecision) -> SceneApprovalDecision {
    ...
    match d {
        ...
        AcceptWithModification {
            ...
            modified_text: modified_dialogue,
        },
        Reject { feedback } => SceneApprovalDecision::Reject { reason: feedback },
        TakeOver { dm_response } => SceneApprovalDecision::ApproveWithEdits {
            modified_text: dm_response,
        },
        Unknown => SceneApprovalDecision::Reject {
            reason: "Unknown approval decision".to_string(),
        },
    }
}
