...
pub async fn handle_staging_approval_response(
    ...
    match state.staging_use_case().approve(ctx, input).await {
...
pub async fn handle_staging_regenerate_request(
    ...
    match state.staging_use_case().regenerate(ctx, input).await {
        Ok(result) => {
...
pub async fn handle_pre_stage_region(
    ...
    match state.staging_use_case().pre_stage(ctx, input).await {
