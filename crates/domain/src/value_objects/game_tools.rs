...
impl GameTool {
    ...
    ///
    /// This is the single source of truth for tool descriptions, used by both:
    /// - `tool_extractor.rs` when processing LLM tool calls
    /// - The `description()` method when working with parsed `GameTool` instances
    pub fn describe_from_json(name: &str, arguments: &serde_json::Value) -> String {
        match name {
            "give_item" => {
...
// Helper functions for describe_from_json
