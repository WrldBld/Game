...
//!
//! This mock implements the full `GameConnectionPort` trait, providing:
//! - Configurable connection state
//! - Recording of all outbound calls for assertions
//! - Ability to emit events to registered callbacks
//!
//! # Example
//!
//! ```ignore
...
//!
//! // Use in tests
//! conn_dyn.send_action("talk", Some("npc-1"), Some("Hello")).unwrap();
//!
//! // Assert outbound calls
//! let sent = conn.sent_actions();
//! assert_eq!(sent.len(), 1);
//! assert_eq!(sent[0].action_type, "talk");
//! ```
...
/// Record of a scene change request
#[derive(Debug, Clone)]
...
impl MockGameConnectionPort {
    /// Create a new mock with the given URL
    ...
    /// Set the connection state and notify any registered callback
    ...
    /// Get all sent actions for assertion
    pub fn sent_actions(&self) -> Vec<SentAction> {
        self.state.lock().unwrap().sent_actions.clone()
    }

    /// Get all sent join requests for assertion
    pub fn sent_joins(&self) -> Vec<SentJoin> {
        self.state.lock().unwrap().sent_joins.clone()
    }

    /// Get all sent scene changes for assertion
    pub fn sent_scene_changes(&self) -> Vec<SentSceneChange> {
        self.state.lock().unwrap().sent_scene_changes.clone()
    }

    /// Get all sent approvals for assertion
    pub fn sent_approvals(&self) -> Vec<SentApproval> {
        self.state.lock().unwrap().sent_approvals.clone()
    }

    /// Get all sent challenge triggers for assertion
    pub fn sent_challenge_triggers(&self) -> Vec<SentChallengeTrigger> {
        self.state.lock().unwrap().sent_challenge_triggers.clone()
    }

    /// Get all sent rolls for assertion
    pub fn sent_rolls(&self) -> Vec<(String, i32)> {
        self.state.lock().unwrap().sent_rolls.clone()
    }
}
