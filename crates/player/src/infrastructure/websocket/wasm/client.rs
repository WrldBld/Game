...
pub struct EngineClient {
    ...
    pending_requests: Rc<RefCell<PendingRequests>>,
    ...
    backoff: Rc<RefCell<BackoffState>>,
}
...
impl EngineClient {
    pub fn new(url: impl Into<String>) -> Self {
        Self {
            ...
            pending_requests: Rc::new(RefCell::new(PendingRequests::default())),
            ...
            backoff: Rc::new(RefCell::new(BackoffState::default())),
        }
    }
    ...
    fn connect_internal(&self) -> Result<()> {
        ...
        *self.closures.borrow_mut() = None;
        ...
        let backoff = Rc::clone(&self.backoff);
                ...
                if let Some(ref ws) = *ws_for_open.borrow() {
                    while let Some(msg) = buffer.pop_front() {
                        ...
                        if let ClientMessage::Request { request_id, .. } = &msg {
                            if !pending_requests_for_open.borrow().contains(request_id) {
                                continue;
                            }
                        }
    ...
    async fn reconnect_with_backoff(&self) {
        ...
        let attempts = self.backoff.borrow().attempts();
        let Some(delay) = self.backoff.borrow_mut().next_delay_and_advance() else {
        ...
        TimeoutFuture::new(delay.min(u32::MAX as u64) as u32).await;
    ...
    pub fn disconnect(&self) {
        ...
        *self.intentional_disconnect.borrow_mut() = true;
        ...
        {
            let count = self.pending_requests.borrow_mut().clear();
            if count > 0 {
                web_sys::console::log_1(
