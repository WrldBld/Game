...
pub fn WorldSelectView(props: WorldSelectViewProps) -> Element {
    ...
    let title = match props.role {
        ParticipantRole::DungeonMaster => "Select World",
        ParticipantRole::Player => "Join a World",
        ParticipantRole::Spectator => "Watch a World",
    };

    let subtitle = match props.role {
        ParticipantRole::DungeonMaster => "Continue an existing campaign or create a new one",
        ParticipantRole::Player => "Choose a world to join as a player",
        ParticipantRole::Spectator => "Choose a world to watch",
    };

    let action_label = match props.role {
        ParticipantRole::DungeonMaster => "Continue",
        ParticipantRole::Player => "Join",
