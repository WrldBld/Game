...
async fn test_thornhaven_world_seeded_correctly() {
    ...
    let mut result = ctx
        .harness
        .graph()
        .execute(query("MATCH (w:World {id: $id}) RETURN w.name as name").param("id", ctx.world.world_id.to_string()))
        .await
        .expect("World query failed");

    let row = result.next().await.expect("No world found").expect("Row error");
    ...
    let mut result = ctx
        .harness
        .graph()
        .execute(query("MATCH (l:Location)-[:LOCATED_IN]->(:World {id: $id}) RETURN count(l) as count").param("id", ctx.world.world_id.to_string()))
        .await
    ...
    let row = result.next().await.expect("No result").expect("Row error");
    let count: i64 = row.get("count").expect("count column");
    assert!(count >= 10, "Should have at least 10 regions, got {}", count);

    // Verify 8 NPCs exist
    let mut result = ctx
        .harness
        .graph()
        .execute(query("MATCH (c:Character)-[:BELONGS_TO]->(:World {id: $id}) RETURN count(c) as count").param("id", ctx.world.world_id.to_string()))
        .await
    ...
    assert!(ctx.world.npc("Marta Hearthwood").is_some(), "Marta should exist");
    assert!(ctx.world.npc("Grom Ironhand").is_some(), "Grom should exist");
    assert!(ctx.world.npc("Old Tom").is_some(), "Old Tom should exist");

    // Verify key locations exist
    assert!(ctx.world.location("The Drowsy Dragon Inn").is_some(), "Inn should exist");
    assert!(ctx.world.location("The Old Mill").is_some(), "Mill should exist");
    assert!(ctx.world.location("Thornhaven Square").is_some(), "Square should exist");
...
async fn test_player_joins_world_at_starting_location() {
    ...
    let row = result.next().await.expect("No player found").expect("Row error");
    let name: String = row.get("name").expect("name column");
    ...
    let row = result.next().await.expect("No location found").expect("Row error");
    ...
    let row = result.next().await.expect("No world relationship").expect("Row error");
...
async fn test_staging_npcs_with_work_schedules() {
    ...
    let inn_id = ctx.world.location("The Drowsy Dragon Inn").expect("Inn should exist");
    ...
    assert!(workers.contains(&"Marta Hearthwood".to_string()),
        "Marta should work at the Inn, got: {:?}", workers);
}
...
async fn test_npcs_frequency_relationships() {
    ...
    assert!(grom_frequents.is_some(), "Grom should have FREQUENTS relationship");

    if let Some((_, time)) = grom_frequents {
        assert!(time.to_lowercase().contains("evening"),
            "Grom should frequent in evening, got: {}", time);
    }
}
...
async fn test_challenges_exist() {
    ...
    assert!(challenges.len() >= 3, "Should have at least 3 challenges, got {}", challenges.len());

    // Verify key challenges exist
    let challenge_names: Vec<&str> = challenges.iter().map(|(n, _)| n.as_str()).collect();
    assert!(challenge_names.contains(&"Convince Grom to Share His Past"),
...
async fn test_narrative_events_exist() {
    ...
    assert!(events.len() >= 3, "Should have at least 3 narrative events, got {}", events.len());

    // Verify key events exist
    let event_names: Vec<&str> = events.iter().map(|(n, _)| n.as_str()).collect();
    assert!(event_names.contains(&"The Stranger's Warning"),
...
async fn test_full_world_structure_integrity() {
    ...
    // Verify all core entities have correct relationships to the world

    // 1. All locations belong to the world
    let mut result = ctx
        .harness
        .graph()
        .execute(
    ...
    // 2. All regions belong to a location
    let mut result = ctx
        .harness
        .graph()
        .execute(
            query(
                "MATCH (r:Region)
                 WHERE NOT (r)-[:LOCATED_IN]->(:Location)
    ...
    // 3. All characters belong to the world
    let mut result = ctx
        .harness
        .graph()
        .execute(
    ...
    // 4. All acts belong to the world
    let mut result = ctx
        .harness
        .graph()
        .execute(
    ...
    let mut result = ctx
        ...
        .execute(
            query(
                "MATCH (s:Scene)
                 WHERE NOT (s)-[:PART_OF]->(:Act)
                 RETURN count(s) as orphan_count"
            ),
        )
        .await
        .expect("Orphan scene query failed");

    let row = result.next().await.expect("No result").expect("Row error");
    let orphan_count: i64 = row.get("orphan_count").expect("orphan_count column");
    assert_eq!(orphan_count, 0, "All scenes should belong to an act");
}
