...
use std::sync::Arc;
use wrldbldr_domain::{self as domain, GameTime, TimeAdvanceReason, TimeAdvanceResult, TimeMode, WorldId};
...
impl World {
    ...
    pub async fn get_current_time(&self, id: WorldId) -> Result<GameTime, WorldError> {
        let world = self.repo.get(id).await?
    ...
    pub async fn advance_time(
        ...
        let mut world = self.repo.get(id).await?
            .ok_or(WorldError::NotFound(id))?;
