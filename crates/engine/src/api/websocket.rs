...
async fn handle_request(
    ...
    let result = match payload {
        // World queries
        RequestPayload::ListWorlds => match state.app.entities.world.list_all().await {
            Ok(worlds) => {
        ...
        RequestPayload::AdvanceGameTime {
            world_id: req_world_id,
            hours,
        } => {
            ...
            let world_id_typed =
                match parse_uuid_for_request(&req_world_id, &request_id, "Invalid world ID") {
        ...
        RequestPayload::AdvanceGameTimeMinutes {
            world_id: req_world_id,
            minutes,
            reason,
        } => {
        ...
        RequestPayload::UpdateTimeConfig {
            ...
            // Normalize: Auto is intentionally treated as Suggested.
        ...
        RequestPayload::GetNpcMood { npc_id, region_id } => {
            ...
            match state
                ...
                .await
            {
                Ok(mood) => ResponseResult::success(serde_json::json!({
                    "npc_id": npc_id,
                    "region_id": region_id,
                    "mood": mood.to_string(),
...
async fn handle_staging_approval(
    ...
    match state
        ...
        .approval
        .approve_staging
        .execute(region_id, npc_ids.clone())
        .await
    {
        Ok(_result) => {
            ...
            if let Some(world_id) = conn_info.world_id {
                ...
                let visual_state =
                    build_visual_state_for_staging(state, location_id, region_id).await;
                ...
                state
                    .connections
                    .broadcast_to_world(world_id, staging_ready)
                    .await;
            }
