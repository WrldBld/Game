...
impl Neo4jRegionStateRepo {
    ...
    fn row_to_state(&self, row: Row) -> Result<RegionState, RepoError> {
        ...
        let id: RegionStateId = parse_typed_id(&node, "id").map_err(|e| {
            RepoError::database("query", format!("Failed to parse RegionStateId: {}", e))
        })?;
        let region_id: RegionId = parse_typed_id(&node, "region_id").map_err(|e| {
            RepoError::database(
                "query",
                format!("Failed to parse region_id for RegionState {}: {}", id, e),
            )
        })?;
        let location_id: LocationId = parse_typed_id(&node, "location_id").map_err(|e| {
            RepoError::database(
                "query",
                format!("Failed to parse location_id for RegionState {}: {}", id, e),
            )
        })?;
        let world_id: WorldId = parse_typed_id(&node, "world_id").map_err(|e| {
            RepoError::database(
                "query",
                format!("Failed to parse world_id for RegionState {}: {}", id, e),
            )
        })?;
        let name_str: String = node.get("name").map_err(|e| {
            RepoError::database(
                "query",
                format!("Failed to get 'name' for RegionState {}: {}", id, e),
            )
        })?;
        let name = StateName::new(&name_str).map_err(|e| {
            RepoError::database(
                "parse",
                format!(
                    "Failed to parse name '{}' for RegionState {}: {}",
                    name_str, id, e
                ),
            )
        })?;
        ...
        let backdrop_override: Option<AssetPath> = node
            ...
            .map_err(|e| {
                RepoError::database(
                    "parse",
                    format!(
                        "Failed to parse backdrop_override for RegionState {}: {}",
                        id, e
                    ),
                )
            })?;
        let atmosphere_override: Option<Atmosphere> = node
            ...
            .map_err(|e| {
                RepoError::database(
                    "parse",
                    format!(
                        "Failed to parse atmosphere_override for RegionState {}: {}",
                        id, e
                    ),
                )
            })?;
        let ambient_sound: Option<AssetPath> = node
            ...
            .map_err(|e| {
                RepoError::database(
                    "parse",
                    format!(
                        "Failed to parse ambient_sound for RegionState {}: {}",
                        id, e
                    ),
                )
            })?;
        ...
        let activation_rules_str =
            node.get_optional_string("activation_rules")
                .ok_or_else(|| {
                    RepoError::database(
                        "query",
                        format!("Missing activation_rules for RegionState {}", id),
                    )
                })?;
        let activation_rules: Vec<ActivationRule> = serde_json::from_str(&activation_rules_str)
            .map_err(|e| {
                RepoError::database(
                    "parse",
                    format!(
                        "Invalid activation_rules JSON for RegionState {}: {} (value: '{}')",
        ...
        let activation_logic_str =
            node.get_optional_string("activation_logic")
                .ok_or_else(|| {
                    RepoError::database(
                        "query",
                        format!("Missing activation_logic for RegionState {}", id),
                    )
                })?;
        let activation_logic: ActivationLogic = serde_json::from_str(&activation_logic_str)
            .map_err(|e| {
                RepoError::database(
                    "parse",
                    format!(
                        "Invalid ActivationLogic JSON for RegionState {}: {} (value: '{}')",
