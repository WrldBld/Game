    ...
    pub async fn process_action_item(&self, action: &DmActionData) -> Result<DmActionResult> {
        ...
        match &action.action {
            DmActionType::ApprovalDecision {
                ...
                // Convert domain DmApprovalDecision to wire format (internal)
                let dto_decision: DmApprovalDecision =
